...
```{r}
select(flights, 19,15,everything())

```

The selection Helper functions help match a pattern
...
```{r}
select(flights, matches("sched"))

```

Differences between matches function and contains function: Matches function can match a regular expression while contains function can only match a literal string.
```{r}
select(flights, matches("sched"))
...
```{r}
select(flights, contains('arr_\\w'))

```

```{r}
select(flights,matches('arr_\\w'))
...
```{r}
df=data.frame(col1=1:5,col2=2:6,col3=4,col4=9,col5=67)
df
```

```{r}
select(df, num_range('col',2:4))
...
- Add variables `gain` and `speed`:
    ...
    ```
    mutate function adds new variable and also keeps the original variables.
    ```{r}
    mutate(flights_sml,
...
- `mutate_at()`: apply funs to specific columns.
    ```{r,eval = FALSE}
    df1<-mutate(flights_sml, gain = arr_delay - dep_delay, speed = distance / air_time * 60 )
    mutate_at(df1, vars(speed), funs(round)) 
    ```

- `mutate_if()`: apply funs of one type
    ```{r,eval = FALSE}
    mutate_if(df1, is.numeric, funs(log(.))) 
    ```
...
- Consider following analysis:
    ...
    delay <- summarise(by_dest, count = n(), # n() - Context dependent expressions: gives the current group size
...
- Position: `first(x)`, `nth(x, 2)`, `last(x)`.
    ...
    not_cancelled %>%
      group_by(year, month, day) %>%
      summarise(
        first_dep = first(dep_time),     # Extract the first value from a vector
        last_dep = last(dep_time)     # Extract the last value from a vector
      )
    ```
...
- Count: `n(x)`, `sum(!is.na(x))`, `n_distinct(x)`.
    ```{r}
    # Which destinations have the most carriers (Two-letter abbreviation of the airline's name)?
    not_cancelled %>%
      group_by(dest) %>%
      summarise(carriers = n_distinct(carrier)) %>%     # Efficiently count the number of unique values in a set of vectors
...
-
    ...
    not_cancelled %>%
      count(tailnum, wt = distance)     # supply wt to perform weighted counts
    ```
...
employee <- data.frame(
  ...
  emp_aff = c("STATS", "BIOS", "MATH", "APPL", "MATH", "STATS", "SIE", "BIOS", "MATH", "APPL", "BIOS")
)
```
